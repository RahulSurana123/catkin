...
//       sync_(message_filters::Synchronizer<SyncPolicy>(SyncPolicy(10), left_sf_, right_sf_)) {
...
{
    SLAM_.startup();
    color_sf_.subscribe(it_, "/camera/rgb/image_raw", 1);
    depth_sf_.subscribe(it_, "/camera/depth/image_raw", 1);

    // sync_= new message_filters::Synchronizer<SyncPolicy(10)>();
        
    sync_.connectInput(color_sf_, depth_sf_);
    sync_.init();
    sync_.registerCallback(boost::bind(&rgbd::callback, this, _1, _2));
}
